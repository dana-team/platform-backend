...
func Test_GetCappLogs(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"ShouldStreamLogsWithoutQueryParams": {
			args: args{
				token: "valid_token",
				wsUrl: fmt.Sprintf("/ws/namespaces/%s/capps/%s/logs", testNamespaceGetCappLogs, testutils.CappName),
			},
			want: want{
		...
		"ShouldNotStreamLogsWithInvalidCappName": {
			args: args{
				token: "valid_token",
				wsUrl: fmt.Sprintf("/ws/namespaces/%s/capps/invalid-capp/logs", testNamespaceGetCappLogs),
			},
			want: want{
		...
		"ShouldStreamLogsWithQueryParams": {
			args: args{
				token: "valid_token",
				wsUrl: fmt.Sprintf("/ws/namespaces/%s/capps/%s/logs?podName=test-pod-2", testNamespaceGetCappLogs, testutils.CappName),
			},
			want: want{
				statusCode:    http.StatusSwitchingProtocols,
				expectedLines: []string{fmt.Sprintf("Capp: %q line: fake logs", testutils.CappName)},
		...
		"ShouldStreamLogsWithPreviousQueryParam": {
			args: args{
				token: "valid_token",
				wsUrl: fmt.Sprintf("/ws/namespaces/%s/capps/%s/logs?previous=true", testNamespaceGetCappLogs, testutils.CappName),
			},
			want: want{
				statusCode:    http.StatusSwitchingProtocols,
				expectedLines: []string{fmt.Sprintf("Capp: %q line: fake logs", testutils.CappName)},
		...
		"ShouldNotStreamLogsWithNonExistingPodName": {
			args: args{
				token: "valid_token",
				wsUrl: fmt.Sprintf("/ws/namespaces/%s/capps/%s/logs?podName=pod%s", testNamespaceGetCappLogs, testutils.CappName, testutils.NonExistentSuffix),
			},
			want: want{
				statusCode:    http.StatusSwitchingProtocols,
		...
		"ShouldNotStreamLogsWithInvalidContainerName": {
			args: args{
				token: "valid_token",
				wsUrl: fmt.Sprintf("/ws/namespaces/%s/capps/%s/logs?containerName=container%s", testNamespaceGetCappLogs, testutils.CappName, testutils.NonExistentSuffix),
			},
			want: want{
				statusCode:    http.StatusSwitchingProtocols,
		...
		"ShouldStreamLogsWithValidContainerName": {
			args: args{
				token: "valid_token",
				wsUrl: fmt.Sprintf("/ws/namespaces/%s/capps/%s/logs?containerName=test-container", testNamespaceGetCappLogs, testutils.CappName),
			},
			want: want{
				statusCode:    http.StatusSwitchingProtocols,
				expectedLines: []string{fmt.Sprintf("Capp: %q line: fake logs", testutils.CappName)},
...
func Test_GetPodLogs(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"ShouldStreamLogsWithoutQueryParams": {
			args: args{
				token: "valid_token",
				wsUrl: fmt.Sprintf("/ws/namespaces/%s/pods/%s/logs", testNamespaceGetPodLogs, pod1),
			},
			want: want{
				statusCode:    http.StatusSwitchingProtocols,
		...
		"ShouldStreamLogsWithQueryParams": {
			args: args{
				token: "valid_token",
				wsUrl: fmt.Sprintf("/ws/namespaces/%s/pods/%s/logs?container=test-container", testNamespaceGetPodLogs, pod1),
			},
			want: want{
				statusCode:    http.StatusSwitchingProtocols,
				expectedLines: []string{fmt.Sprintf("Pod: %q line: fake logs", pod1)},
		...
		"ShouldStreamLogsWithPreviousQueryParam": {
			args: args{
				token: "valid_token",
				wsUrl: fmt.Sprintf("/ws/namespaces/%s/pods/%s/logs?previous=true", testNamespaceGetPodLogs, pod1),
			},
			want: want{
				statusCode:    http.StatusSwitchingProtocols,
				expectedLines: []string{fmt.Sprintf("Pod: %q line: fake logs", pod1)},
		...
		"ShouldStreamLogsWithoutQueryParamsMultipleContainers": {
			args: args{
				token: "valid_token",
				wsUrl: fmt.Sprintf("/ws/namespaces/%s/pods/%s/logs", testNamespaceGetPodLogs, pod3),
			},
			want: want{
				statusCode:    http.StatusSwitchingProtocols,
		...
		"ShouldNotStreamLogsWithNonExistingPodName": {
			args: args{
				token: "valid_token",
				wsUrl: fmt.Sprintf("/ws/namespaces/%s/pods/test-invalid-pod/logs", testNamespaceGetPodLogs),
			},
			want: want{
				statusCode:    http.StatusSwitchingProtocols,
		...
		"ShouldNotStreamLogsWithInvalidContainerName": {
			args: args{
				token: "valid_token",
				wsUrl: fmt.Sprintf("/ws/namespaces/%s/pods/%s/logs?containerName=container%s", testNamespaceGetPodLogs, pod1, testutils.NonExistentSuffix),
			},
			want: want{
				statusCode:    http.StatusSwitchingProtocols,
